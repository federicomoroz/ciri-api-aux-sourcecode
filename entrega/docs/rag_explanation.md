...
compartir resolucion. No es asi. Dos casos pueden ser 0.95 similares —mismo comercio, mismo
metodo de pago, montos parecidos— y merecer resoluciones opuestas, porque lo que decide el caso
son los veredictos de politica sobre *esa* transaccion y el historial de *ese* cliente.

Lo que si existe es un cache de idempotencia exacto en SQLite, con clave
`(transaction_id, cliente_vip)`. Investigar dos veces la misma transaccion devuelve el informe ya
generado: **113 segundos la primera vez, 2 segundos la segunda**. La ganancia de no repetir
trabajo se obtiene igual, sin el riesgo de devolver el razonamiento de otro caso.

El razonamiento completo, con lo que se haria distinto en produccion, esta en `decisions.md#9`.
